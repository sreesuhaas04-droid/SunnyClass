"""Render the app in a headless browser with a fake camera and capture previews."""
import sys, time, pathlib
from playwright.sync_api import sync_playwright

OUT = pathlib.Path("/home/claude/smartclass/preview"); OUT.mkdir(exist_ok=True)
...
with sync_playwright() as p:
    ...
    # ---------- classroom join gate (student, no real face on the fake cam) ----------
    ctx, page = new_ctx(1440, 900)
    page.goto(BASE, wait_until="networkidle")
    page.click("#fill-student"); page.click("#auth-submit")
    page.wait_for_url("**/dashboard.html", timeout=15000)
    page.wait_for_selector("#btn-join:not([hidden])", timeout=15000)
    page.click("#btn-join")
    page.wait_for_url("**/app.html**", timeout=15000)
    time.sleep(16)
    shot(page, "06-classroom-gate")
    print("  gate message:", page.inner_text("#gate-msg")[:140])
    ctx.close()

    # ---------- classroom itself (teacher hosts, so no face gate) ----------
    ctx, page = new_ctx(1440, 900)
    page.goto(BASE, wait_until="networkidle")
    page.fill("#f-email", "ramesh.iyer@smartclass.edu"); page.fill("#f-pass", "teach1234")
    page.click("#auth-submit")
    page.wait_for_url("**/dashboard.html", timeout=15000)
    ...
    ctx, page = new_ctx(1440, 900)
    page.goto(BASE, wait_until="networkidle")
    page.fill("#f-email", "ramesh.iyer@smartclass.edu")
    ...
    # ---------- tablet ----------
    ctx, page = new_ctx(834, 1112, mobile=True)
    page.goto(BASE, wait_until="networkidle")
    page.click("#fill-student"); page.click("#auth-submit")
    page.wait_for_url("**/dashboard.html", timeout=15000)
    page.wait_for_selector("#stats .stat-value", timeout=15000)
    time.sleep(2.5)
    shot(page, "11-dashboard-tablet", full=True)
    ctx.close()
